...
function AccountMainPage() {
  ...
  const shouldshow_vendormodal = useSelector(
    (state) => state.newvendor.addvendor
  );
  useEffect(() => {
    ...
    fetch(`${server_url}admin/vendor/null?token=${token}`, {
      method: "GET",
      headers: {
    ...
    <React.Fragment>
      ...
      <div className="container my-3">
        <button
          ...
          onClick={() => dispatch(ShowAddvendorModal())}
        >
          Add vendor
        </button>
      </div>
      {/* {showvendormodel?<Vendormodel/>:null}
         ...
         */}
      <Vendormodel
        show={shouldshow_vendormodal}
        onHide={() => dispatch(ShowAddvendorModal())}
      />
    </React.Fragment>
  );
}
