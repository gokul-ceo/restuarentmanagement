...
function EmployeeAddsection(props){
    ...
    return <>
     <Offcanvas  show={props.show} onHide={props.handleclose} backdrop="static">
        ...
        <Offcanvas.Body>
            {iserror &&  <div class="alert alert-danger fs-6 p-2" role="alert">
                    Something went wrong :(
            </div> }
          <div>
            <form onSubmit={props.submit} >
            <div className="form-floating mb-3">
                <input required type='text' onChange={props.nameaction} value={props.namevalue} id="floatingnameinput" placeholder="enter employee name.." className="form-control"/>
                <label htmlFor="floatingnameinput" >Employee Name</label>
            </div>
            <div className="row">
                <div className="col">
                <div className="form-floating mb-3">
                <input required type='Number' onChange={props.mobilaction} value={props.mobilevalue} id="floatingcontactinput" placeholder="enter employee name.." className="form-control"/>
                <label htmlFor="floatingcontactinput" >Employee mobile</label>
                 </div>
                </div>
                <div className="col">
                <div class="form-floating">
  <select onChange={props.genderaction}  className="form-select" id="floatingSelect">
  ...
  <label htmlfor="floatingSelect">Select Gender</label>
            ...
            <div className="form-floating mb-3">
                <input onChange={props.aadharaction} value={props.aadharvalue} required type='Number' id="floatingcontactinput" placeholder="enter employee name.." className="form-control"/>
                <label htmlFor="floatingcontactinput" >Employee Aadhar</label>
                 </div>
                 <div class="form-floating">
  <select onChange={props.roleaction} class="form-select" id="floatingSelect">
    <option value="0" selected >select</option>
  ...
  <div className="my-2">
  <label for="empaddressTextarea1" class="form-label">Employee Address</label>
  <textarea onChange={props.addressaction} class="form-control" id="empaddressTextarea1" rows="2"></textarea>
</div>
</div>
...
function Employepage(){
    ...
    const empattendancelist = useSelector((state)=>state.emplist.Empattendancelist)
    const getupdatedemplist = useSelector((state)=>state.emplist.isemplistupdated)
    ...
    async function handlesubmit(e){
        ...
        const response = await fetch('http://localhost:4000/employee/data?operation=add',requestoption);
    ...
    useEffect(()=>{
        fetch('http://localhost:4000/employeedetails/all',{
            method:'GET',
            headers:{
    ...
    function PresentMarkingTab(){
        if( empattendancelist!==undefined && empattendancelist !== []){
            switch (empattendancelist.length) {
                ...
                case 2:
                    return <h6>{empattendancelist[0]},{empattendancelist[1]}</h6>
                case 3:
                    return <h6>{empattendancelist[0]},{empattendancelist[1]},{empattendancelist[2]}</h6>
                case 4:
                    return <h6>{empattendancelist[0]},{empattendancelist[1]},{empattendancelist[2]},{empattendancelist[3]}</h6>
                default:
                    return <h6>{empattendancelist[0]},{empattendancelist[1]},{empattendancelist[2]},{empattendancelist[3]}.. {empattendancelist.length - 4}more</h6>
            }
    ...
    <div className="container">
      ...
      <input onChange={handlesearch} value={query}  type="text" class="form-control"  placeholder='type to search..'/>
      <button className="btn border-dark " type="button" data-bs-toggle="dropdown" aria-expanded="false"><img alt="option" style={{'width':'20px','height':'20px'}} src={search} /></button>
      <ul className="dropdown-menu dropdown-menu-end">
        <li onClick={handledropdownclick} value='0' className="dropdown-item" >Name</li>
        <li onClick={handledropdownclick} value='1' className="dropdown-item">Empid</li>
        {/* <li className="dropdown-item">Something else here</li> */}
            ...
            <button className="btn border-dark" onClick={handleshow} type="button" ><img alt="option" style={{'width':'20px','height':'20px'}} src={addemp} /></button>
            </div>
    ...
    {employeelist[0]===undefined?null:employeelist[0].filter(user=>user.Empname.toLowerCase().includes(query)).map((user)=>(
     <EmployeeBox  gender={user.Empgender} name={user.Empname} />
    ))}
    {/* {Noresult && <div class="alert alert-danger my-1" role="alert">
        No result found..
        </div>} */}
    <EmployeeAddsection onerror={iserror} submit={handlesubmit} namevalue={empname} mobilevalue={empmobil} gendervalue={empgender} aadharvalue={empaadhar} rolevalue={emprole} addressvalue={empaddress} nameaction={handlenamechange} mobilaction={handlemobilchange} genderaction={handlegenderchange} aadharaction={handleaadharchange} roleaction={handleemprole} addressaction={handleempaddress} show={show} handleclose={handleshow} />
